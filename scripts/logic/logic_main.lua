...
manual_legostarwarsthecompletesaga_mysteryem_location = {}
manual_legostarwarsthecompletesaga_mysteryem_location.__index = manual_legostarwarsthecompletesaga_mysteryem_location
...
function manual_legostarwarsthecompletesaga_mysteryem_location.new(name)
    local self = setmetatable({}, manual_legostarwarsthecompletesaga_mysteryem_location)
    if name then
...
function manual_legostarwarsthecompletesaga_mysteryem_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = manual_legostarwarsthecompletesaga_mysteryem_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function manual_legostarwarsthecompletesaga_mysteryem_location:connect_two_ways(exit, rule)
...
function manual_legostarwarsthecompletesaga_mysteryem_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function manual_legostarwarsthecompletesaga_mysteryem_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function manual_legostarwarsthecompletesaga_mysteryem_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function manual_legostarwarsthecompletesaga_mysteryem_location:accessibility()
...
-- 
function manual_legostarwarsthecompletesaga_mysteryem_location:discover(accessibility, keys)
...
entry_point = manual_legostarwarsthecompletesaga_mysteryem_location.new("entry_point")
